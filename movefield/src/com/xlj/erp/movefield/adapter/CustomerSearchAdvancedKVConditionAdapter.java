...
public class CustomerSearchAdvancedKVConditionAdapter extends BaseAdapter {
	...
	public CustomerSearchAdvancedKVConditionAdapter(Context context, List<KeyValue> conditions, List<Boolean> checkedList) {
		mContext = context;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.tvCondition.setText(mConditions.get(position).getValue());
	...
	public String getChecked() {
		...
		for (int i = 0; i < mCheckedList.size(); i++) {
			if (mCheckedList.get(i)) {
				result = mConditions.get(i).getKey();
				break;
			}
		}
		return result;
